...
const SHIPPING_BY_COUNTRY: Record<FeedCountry, {
  ...
  GB: { service: 'Free UK Standard Delivery', currency: 'GBP' },
};
  ...
  try {
    ...
    const itemsXml = products
      ...
      .map((product) => {
        ...
        const brand = escapeXml(product.brand || 'OpticMart');
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>OpticMart Google Merchant Center Feed${targetLabel}${currencyLabel}</title>
    <link>${BASE_URL}</link>
    <description>Selected OpticMart products for ${country || 'United States'}${currencyLabel}</description>
    ${itemsXml}
  </channel>
</rss>`;
