...
const BASE_URL = 'https://opticmart.shop';
...
export async function generateMetadata(
  ...
  try {
    ...
    if (!slug) return { title: 'Product Not Found | OpticMart' };

    let product = isReviewProduct(slug) ? getReviewProduct(slug) : null;
    if (!product) product = await getProductBySlug(slug);
    if (!product || !isPublicStoreProduct(product)) return { title: 'Product Not Found | OpticMart', robots: { index: false, follow: false } };

    const title = `${product.title || 'Product'} - ${product.brand || ''} | ${product.category || ''} | OpticMart`;
    ...
    return {
      title: 'Product | OpticMart',
      description: 'Browse our products on OpticMart',
    };
  }
}
