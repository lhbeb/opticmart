...
export default function SellerBadge({ sellerId, size = 'sm' }: SellerBadgeProps) {
  ...
  const displaySeller = seller || {
    id: 'opticmart',
    name: 'OpticMart Official',
    username: 'opticmart',
    avatarUrl: fallbackAvatarUrl,
  };

  const isOpticMart = displaySeller.username === 'opticmart';
  const href = isOpticMart ? '/' : `/sellers/${displaySeller.username}`;
  ...
  if (size === 'sm') {
    return (
      ...
      >
        ...
        {isOpticMart ? (
          <ShieldCheck className="w-3 h-3 flex-shrink-0 text-[#0284C7]" />
        ) : (
          <Star className="w-3 h-3 flex-shrink-0 text-[#0284C7] fill-[#0284C7]" />
        )}
      </Link>
    );
  }
  ...
  return (
    ...
    >
      {/* Avatar / icon */}
      <div className="w-5 h-5 rounded-full overflow-hidden bg-[#F8FAFC] flex items-center justify-center flex-shrink-0 ring-1 ring-[#0284C7]/30 group-hover:ring-[#0F172A]/50 transition-all">
        {hasAvatar ? (
      ...
      <div className="relative group flex items-center">
        {isOpticMart ? (
          <ShieldCheck className="w-3.5 h-3.5 flex-shrink-0 text-[#0284C7]" />
        ) : (
          <Star className="w-3.5 h-3.5 flex-shrink-0 text-[#0284C7] fill-[#0284C7] cursor-help" />
        )}
      ...
      {seller && (seller.totalReviews ?? 0) > 0 && (
        <div className="flex items-center text-xs text-gray-500 font-medium ml-1">
          ...
          <Star className="w-3 h-3 text-[#0284C7] fill-[#0284C7] mr-1" />
